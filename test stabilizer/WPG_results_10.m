...
%     Fz_=[Fz_;Fz_d(i)];
    ...
    toto1=-((-xp2_(end)*yp_smooth(end)+xp_smooth(end)*yp2_(end))/(xp2_(end)*yp1_(end)-xp1_(end)*yp2_(end)));
    toto2=-((xp1_(end)*yp_smooth(end)-xp_smooth(end)*yp1_(end))/(xp2_(end)*yp1_(end)-xp1_(end)*yp2_(end)));
...
%     end
    displangleact2=[displ_tot2(:,end);angleact_tot2(:,end)]-Jmat.step_2.J_tot((40)*6+1:(40)*6+6,:)^-1*delta_F2;
    ...
    %%
%     delta_F1_=[Fx1(end);Fy1(end);Fz1(end);yp1(end);xp1(end);0]-[Fx1_(i);Fy1_(i);Fz1_(i);yp1_(i);xp1_(i);0];
    %% robot simulation
