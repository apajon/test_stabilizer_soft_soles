...
%     yp_=[yp_;max([min([yp_d(i)+delta_corr_y;wpg_param.inttoankle]);-wpg_param.inttoankle])];
    ...
    xp_smooth=filter(0.01/0.05,[1 0.01/0.05-1],xp_-xp_(1),0)+xp_(1);
...
%     xp2_=[xp2_;xp2_d(i)+delta_corr_x];
    ...
    xp2_=[xp2_;max([min([xp2_d(1)+toto(end);wpg_param.step_number_pankle_fixed(2,2)+wpg_param.fronttoankle]);wpg_param.step_number_pankle_fixed(2,2)-wpg_param.backtoankle])];
...
%     yp2_=[yp2_;yp2_d(i)+delta_corr_y];
    ...
    yp2_=[yp2_;max([min([yp2_d(1)+toto(end);wpg_param.step_number_pankle_fixed(2,3)+wpg_param.inttoankle]);wpg_param.step_number_pankle_fixed(2,3)-wpg_param.inttoankle])];
...
%     end
    displangleact2=[displ_tot2(:,end);angleact_tot2(:,end)]-Jmat.step_2.J_tot((40)*6+1:(40)*6+6,:)^-1*delta_F1;
...
%     yp=[yp;time(i) (yp2(end)*Fz2(end)/Fz(end)+yp1(end))*Fz1(end)/(Fz(end))];
    %%
%     if i>=40 && i<151
%         xax(end,2)=xax(end,2)+2/(wpg_param.m);
