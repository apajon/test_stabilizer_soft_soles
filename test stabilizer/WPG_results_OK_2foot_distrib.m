...
%     xp1_=[xp1_;xp1_d(i)];
%     xp2_=[xp2_;xp2_d(i)];
%     Fx_=[Fx_;-(xx(end)-xp_smooth(end))*w^2*wpg_param.m];
...
%     toto=filter((1/f)/0.05,[1 (1/f)/0.05-1],xp_-xp_(1),0);
    ...
    yp1_=[yp1_;yp1_d(i)];
    yp2_=[yp2_;yp2_d(i)];
%     Fy_=[Fy_;-(yx(end)-yp_smooth(end))*w^2*wpg_param.m];
    
%     Fz_=[Fz_;Fz_d(i)];
    
    toto1=max(min(-((-xp2_(end)*yp_smooth(end)+xp_filtered(end)*yp2_(end))/(xp2_(end)*yp1_(end)-xp1_(end)*yp2_(end))),0.97),0.03);
    toto2=max(min(-((xp1_(end)*yp_smooth(end)-xp_filtered(end)*yp1_(end))/(xp2_(end)*yp1_(end)-xp1_(end)*yp2_(end))),0.97),0.03);
    Fz1_=[Fz1_;Fz_(1)*toto1];
    Fz2_=[Fz2_;Fz_(1)*toto2];   
    
    yp_filtered=(yp1_.*Fz1_+yp2_.*Fz2_)/Fz1_d(end);    
    Fy_=[Fy_;-(yx(end)-yp_filtered(end))*w^2*wpg_param.m];
    

    Fx1_=[Fx1_;Fx_(i)*toto1];
    Fx2_=[Fx2_;Fx_(i)*toto2];
    Fy1_=[Fy1_;Fy_(i)*toto1];
    Fy2_=[Fy2_;Fy_(i)*toto2];
...
%     lambda=100;
%     mu=0.01;
%     func = @(pbc_) (abs(sum(pbc_))+abs(sum(pbc_-pbc_tot(end))/2));%pbc_=[dFz1;dFz2;dxp1;dxp2;dyp1;dyp2;]
% 
%     pbc0=[0;0;0;0;0;0];
%     A=[];
%     b=[];
%     Aeq=[1 1 0 0 0 0];
%     beq=[0];
%     lb=[-Fz1_d(i);-Fz2_d(i);-wpg_param.backtoankle+wpg_param.sole_margin;-wpg_param.backtoankle+wpg_param.sole_margin;-wpg_param.exttoankle+wpg_param.sole_margin;-wpg_param.inttoankle+wpg_param.sole_margin];
%     ub=[+Fz1_d(i);+Fz2_d(i);+wpg_param.fronttoankle-wpg_param.sole_margin;+wpg_param.fronttoankle-wpg_param.sole_margin;+wpg_param.inttoankle-wpg_param.sole_margin;+wpg_param.exttoankle-wpg_param.sole_margin];
%     nonlcon = @(pbc_) confun(pbc_,Fz_d(i),xp_smooth(end),yp_smooth(end),Fz1_d(i),xp1_d(i),yp1_d(i),Fz2_d(i),xp2_d(i),yp2_d(i));
%     
% %     opt = optimset('Algorithm', 'sqp','DiffMinChange',10^-5,'Display','iter');
% %     opt = optimset('Algorithm', 'sqp','DiffMinChange',10^-7,'Display','off');
...
%     pbc_tot=[pbc_tot;pbc'];
% 
% %     xp1_(end)=xp1_d(1)+pbc(3);
% %     xp2_(end)=xp2_d(1)+pbc(4);
% %     yp1_(end)=yp1_d(1)+pbc(5);
% %     yp2_(end)=yp2_d(1)+pbc(6);
% %     Fz1_(end)=Fz1_d(1)+pbc(1);
% %     Fz2_(end)=Fz2_d(1)+pbc(2);
% %     
% %     Fx1_(end)=Fx_(end)*(pbc(1)+Fz1_d(1))/Fz_d(1);
% %     Fx2_(end)=Fx_(end)*(pbc(2)+Fz2_d(1))/Fz_d(1);
% %     Fy1_(end)=Fy_(end)*(pbc(1)+Fz1_d(1))/Fz_d(1);
% %     Fy2_(end)=Fy_(end)*(pbc(2)+Fz2_d(1))/Fz_d(1);
    %%
