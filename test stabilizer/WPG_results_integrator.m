...
k2=-Tp*(1+...
    (poles(1)*poles(2)+poles(1)*poles(3)+poles(1)*poles(4)+...
    poles(2)*poles(3)+poles(2)*poles(4)+...
    poles(3)*poles(4))/w^2);

k1=-k3-Tp*(1/Tp-(poles(1)*poles(2)*poles(3)+poles(1)*poles(2)*poles(4)+...
    poles(1)*poles(3)*poles(4)+poles(2)*poles(3)*poles(4))/w^2);
...
[sole_R param_sopt_R ABe_R Pg_R FcFreeSurf_R D_R]=flexible_sole_model_init(0.65,80000*4,0.3);
...
[sole_L param_sopt_L ABe_L Pg_L FcFreeSurf_L D_L]=flexible_sole_model_init(0.65,80000*4,0.3);
...
%     yp_=[yp_;yp_d(i)+delta_corr_y];
    ...
    delta_xx=xx_d(i)-xx(end);
    int_xx=[int_xx;int_xx(end)+delta_xx/f];
    
    delta_yx=yx_d(i)-yx(end);
    int_yx=[int_yx;int_yx(end)+delta_yx/f];
    
    delta_xsx=L(2)*xsx_d(i)-K(2)*xsx(end);
    delta_xx=L(1)*xx_d(i)-K(1)*xx(end)+J(1)*int_xx(end);
    delta_xp=L(3)*xp_d(i)-K(3)*xp(end,2);
    delta_corr_x=delta_xsx+delta_xx+delta_xp;
    xp_=[xp_;delta_corr_x];
    
    delta_ysx=L(2)*ysx_d(i)-K(2)*ysx(end);
    delta_yx=L(1)*yx_d(i)-K(1)*yx(end)+J(1)*int_yx(end);
    delta_yp=L(3)*yp_d(i)-K(3)*yp(end,2);
    delta_corr_y=delta_ysx+delta_yx+delta_yp;
    yp_=[yp_;delta_corr_y];
...
%             [xv yv xvR_reduced yvR_reduced xvL_reduced yvL_reduced xvR yvR xvL yvL]=compute_convexhull_vertices(wpg_param.step_number_pankle_fixed(RFootIndex,:),wpg_param.step_number_pankle_fixed(LFootIndex,:),wpg_param);
    ...
    [xv yv xvR_reduced yvR_reduced xvL_reduced yvL_reduced xvR yvR xvL yvL]=compute_convexhull_vertices(wpg_param.step_number_pankle_fixed(RFootIndex,:),wpg_param.step_number_pankle_fixed(LFootIndex,:),wpg_param);
%     xv=xp(end,2)+(xv-xp(end,2))*Ts*f;
%     yv=yp(end,2)+(yv-yp(end,2))*Ts*f;
%     
%     xvR=xp(end,2)+(xvR-xp(end,2))*Ts*f;
%     yvR=yp(end,2)+(yvR-yp(end,2))*Ts*f;
%     
%     xvL=xp(end,2)+(xvL-xp(end,2))*Ts*f;
%     yvL=yp(end,2)+(yvL-yp(end,2))*Ts*f;

    %% Bring back the ZMP into the Convex hull
%     if k_2<1 && k_2>0 && wpg_param.dt_type_phase(i)==0
...
%     Fz_=[];
    ...
    displangleactR=[toto4(end,:)';toto3(end,:)']-Jmat.step_1.J_tot((40)*6+1:(40)*6+6,:)^-1*delta_FR*(1/f*1/Ts);
    displangleactL=[toto6(end,:)';toto5(end,:)']-Jmat.step_2.J_tot((40)*6+1:(40)*6+6,:)^-1*delta_FL*(1/f*1/Ts);
...
%     toto3=toto3+(angleact_totR(:,end)'-toto3)*1/f/T;
%     toto4=toto4+(displ_totR(:,end)'-toto4)*1/f/T;
%     
%     toto5=toto5+(angleact_totL(:,end)'-toto5)*1/f/T;
%     toto6=toto6+(displ_totL(:,end)'-toto6)*1/f/T;
...
%     simu.yax=simOut.get('yax');
    simu.xx=xx(end)+xsx(end)*1/wpg_param.frequency+xax(end,2)*(1/wpg_param.frequency)^2/2;
    simu.yx=yx(end)+ysx(end)*1/wpg_param.frequency+yax(end,2)*(1/wpg_param.frequency)^2/2;
    simu.zx=zx(end)+zsx(end)*1/wpg_param.frequency+zax(end,2)*(1/wpg_param.frequency)^2/2;
    simu.xsx=xsx(end)+xax(end,2)*1/wpg_param.frequency;
    simu.ysx=ysx(end)+yax(end,2)*1/wpg_param.frequency;
    simu.zsx=zsx(end)+zax(end,2)*1/wpg_param.frequency;
...
plot(time_r,xp_d,'k')
plot(time_r, xx,'--k')
hleg = legend('ZMPR (ED)','ZMPL (ED)','ZMP (ED)','ZMPR* (control)','ZMPL* (control)','ZMP* (control)','ZMP WPG','COM (ED)','Location','East');
% Make the text of the legend italic and color it brown
set(hleg,'FontAngle','italic','TextColor',[.3,.2,.1])
hold off
...
plot(time_r,[ypR_(1:end-3) ypL_(1:end-3) ((FzR_./Fz_).*ypR_(1:end-3)+FzL_./Fz_.*ypL_(1:end-3))],'--')
plot(time_r,yp_d,'k')
plot(time_r, yx,'--k')
hleg = legend('ZMPR (ED)','ZMPL (ED)','ZMP (ED)','ZMPR* (control)','ZMPL* (control)','ZMP* (control)','ZMP WPG','COM (ED)','Location','East');
% Make the text of the legend italic and color it brown
