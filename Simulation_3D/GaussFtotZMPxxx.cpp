...
/*#ifdef _WIN32
  ...
  GS->set_data_output(displ_out,angleact_out,FtotZMPOut,Fc_mc3_out,Pc_mc3_out,ind_cont_out,Kcart_out); // Set data members to incoming
...
void mexFunction(int num_output, mxArray *output[], int num_input, const mxArray *input[])
{
  ...
  } else if (num_output >= 8) {
    mexErrMsgIdAndTxt("MATLAB:Gauss:nargout",
            "Gauss requires 7 output argument.");
  }
  ...
  Gauss(contAngle,fric,m,displ_in1,angleact_in1,FtotZMPdes_in1,F3_in1,Pfree_in1,PabsOld_in1,Ccc_in1,displ_out,angleact_out,FtotZMP_out,Fc_mc3_out,Pc_mc3_out,ind_cont_out,Kcart_out);
