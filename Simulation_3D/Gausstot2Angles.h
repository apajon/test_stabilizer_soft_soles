...
class GaussSeidel {
    public:
      ...
      //void display();
      void set_data_output(double *displ_out,double *angleact_out,double *FtotZMPOut,double *Fc_mc3_out,double *ind_cont_out,double *Kcart_out);
      //void set_data_input(const MatrixXd& Ccc_in);
      void setStaticDataSim(const MatrixXd& Ccc_in,const Vector5d& FtotZMPdes_in,double friction,int m);
      void setStaticDataStep(const Vector3d& displ_in,const Vector2d& angleact_in,const VectorXd& F3_in,const MatrixXd& Pfree_in,const MatrixXd& PabsOld_in);
...
//      void cross(const Ref<MatrixXd>& vec, Ref<MatrixXd> vec_hat);
    ...
    private:
       //double val1, val2;
		...
		const static double epsiZMP; // Desired precision
